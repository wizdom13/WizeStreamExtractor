...
//        }
                ...
                if (info != null) {
                    if (item.itagType == ItagItem.ItagType.AUDIO && format.has("audioTrack")) {
                        ...
                        info.setAudioTrackInfo(id, track.getString("displayName", language),
                                language.split("-")[0],
                                extractAudioTrackType(format.getString("xtags")));
                    }
                    itags.add(info);
                }
            } catch (final Exception ignored) {
        ...
        for (final ItagInfo info : itags) {
            ...
            if (item.itagType == ItagItem.ItagType.AUDIO) {
                final AudioStream stream = new AudioStream.Builder()
                        ...
                        .setAudioTrackType(info.getAudioTrackType())
                        .build();
    ...
    private void parseHlsMasterManifest(@Nonnull final String manifestContent,
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            audioBuilder.setAudioTrackType(extractHlsAudioTrackType(uri));
    ...
    private static AudioTrackType extractHlsAudioTrackType(@Nonnull final String url) {
        try {
            ...
            if (matcher.find()) {
                final String xtags = matcher.group(1);
                ...
                for (final String part : xtags.split(":")) {
                    ...
                    if (keyValue.length == 2 && "acont".equals(keyValue[0])) {
                        return audioTrackTypeFromAudioContentType(keyValue[1]);
                    }
                }
            }
        } catch (final Exception ignored) {
